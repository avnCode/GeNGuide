...
class BaseLearner(object):
    ...
    def save_checkpoint(self, filename, head_only=False):
        ...
        torch.save(save_dict, '{}_{}_ours.pth'.format(filename, self._cur_task))
        print("Saving to....")
        print(filename)
    ...
    def eval_task(self):
        ...
        print(Counter(y_true))  # Prints the counts of elements in y_true
    ...
    def _compute_accuracy(self, model, loader):
        model.eval()
        correct, total = 0, 0
        for i, (_, inputs, targets, _, _) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = model(inputs)['logits']
    ...
    def _extract_vectors(self, loader):
        self._network.eval()
        vectors, targets, ranking = [], [], []
        for _, _inputs, _targets, _ranking, _ in loader:
            _targets = _targets.numpy()
            _ranking = _ranking.numpy()
            if isinstance(self._network, nn.DataParallel):
        ...
        return np.concatenate(vectors), np.concatenate(targets), np.concatenate(ranking)
    ...
    def weighted_cov(self, X, weights, means):
        n_samples, n_features = X.shape
        X_normalized = X - means
        cov_matrix = torch.zeros((n_features, n_features))
        weighted_outer_products = torch.zeros((n_samples, n_features, n_features))
        for i in range(n_samples):
            weighted_outer_products[i] = weights[i] * torch.outer(X_normalized[i], X_normalized[i])
        cov_matrix = torch.sum(weighted_outer_products, dim=0)

        if self.biased_cov:
            denominator = torch.sum(weights)
        else:
            denominator = torch.sum(weights) - 1
    ...
    def _compute_class_mean(self, data_manager, check_diff=False, oracle=False):
        ...
        if check_diff:
            for class_idx in range(0, self._known_classes):
                data, targets, ranking, idx_dataset = data_manager.get_dataset(np.arange(class_idx, class_idx+1), source='train',
                                                                    mode='test', ret_data=True)
                idx_loader = DataLoader(idx_dataset, batch_size=batch_size, shuffle=False, num_workers=4)
                vectors, _, ranking = self._extract_vectors(idx_loader)

                if self.N2:
                    class_mean = np.sum(vectors*ranking[:, np.newaxis], axis=0)/np.sum(ranking)
                    if not self.mean_only:
                        class_cov = self.weighted_cov(torch.tensor(vectors, dtype=torch.float64), torch.tensor(ranking, dtype=torch.float64), class_mean)
                else:
        ...
        if oracle:
            for class_idx in range(0, self._known_classes):
                ...
                if self.N2:
                    class_mean = np.sum(vectors*ranking[:, np.newaxis], axis=0)/np.sum(ranking)
                    if not self.mean_only:
                        class_cov = self.weighted_cov(torch.tensor(vectors, dtype=torch.float64), torch.tensor(ranking, dtype=torch.float64), class_mean) + torch.eye(class_mean.shape[-1])*1e-5
                else:
                    class_mean = np.mean(vectors, axis=0)
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            data, targets, ranking, isNoise, _, idx_dataset = data_manager.get_dataset(np.arange(class_idx, class_idx+1), source='train', mode='test', ret_data=True)
            idx_loader = DataLoader(idx_dataset, batch_size=batch_size, shuffle=False, num_workers=4)
            vectors, _, ranking = self._extract_vectors(idx_loader)
            if self.N2:
                class_mean = np.sum(vectors*ranking[:, np.newaxis], axis=0)/np.sum(ranking)
                if not self.mean_only:
                    class_cov = self.weighted_cov(torch.tensor(vectors, dtype=torch.float64), torch.tensor(ranking, dtype=torch.float64), class_mean) + torch.eye(class_mean.shape[-1])*1e-4
    ...
    def _compute_eta(self, data_manager):        
        clean_avg  = []
        cln_noSamples = 0
        noise_avg = []
        nse_noSamples = 0
        for class_idx in range(self._known_classes, self._total_classes):
            data, targets, ranking, org_targets, _, idx_dataset = data_manager.get_dataset(np.arange(class_idx, class_idx+1), source='train', mode='test', ret_data=True)
            is_match = targets == org_targets
            ranking = ranking/self.neighbors_to_check
            if np.count_nonzero(is_match):
                clean_avg.append(np.sum(ranking * is_match))
                cln_noSamples+=np.count_nonzero(is_match)
            else:
                clean_avg.append(0)

            if np.count_nonzero(~is_match):
                noise_avg.append(np.sum(ranking * (1 - is_match))/np.count_nonzero(~is_match))
                nse_noSamples+=np.count_nonzero(~is_match)

            else:
                noise_avg.append(0)
        log_info = f"Avg eta (clean): {sum(clean_avg)/cln_noSamples}, Avg eta (noisy): {sum(noise_avg)/nse_noSamples}"
        logging.info(log_info)
